...
//Arena: A lot of ppl may get annoyed because it may call this more than once on connection enter...
/*void loginplz (gentity_t *ent) {
	MYSQL *conn;
	MYSQL_RES *res;
	MYSQL_ROW row;
	...
	mysql_query(conn, va("SELECT username,user_password,user_kills,user_deaths,user_experience,duel_wins,duel_losses,flag_captures,flag_returns,siege_objectives,login_ip,ingame_rank FROM phpbb_users WHERE username = '%s' AND user_password = '%s'", buff, buff2 ) );
...
static void ClientCleanName( const char *in, char *out, int outSize ) {
	...
	*p = 0;
	...
	while( 1 ) {
		...
		if( (colorlessLen == 0 || !*p) && ch == ' ' ) { //Color + Space is not a character
			continue;
		}

		if (ch == 255 || ch == -96){ //Character 255 is a blank character (ALT+255)
			continue;
		}
	...
	if ( client->pers.connected == CON_CONNECTED ) {
		if ( strcmp( oldname, client->pers.netname ) ) 
		{
			...
			{				
				...
				G_LogPrintf( "ClientNameChange: Client %i changed name from %s to %s (IP address %s)\n", clientNum, oldname, client->pers.netname, s );
			}
		}
	}
...
//	char		*areabits;
	...
	if ( firstTime ) {
	/* if clever fake detection is activated, check user info */
		if (strcmp(gaPortlessIP, "") == 0 && !isBot) {
			return "You don't have an IP address!";
		}
	}
	
	if (!isBot && (strcmp(Info_ValueForKey(userinfo, "cg_predictItems"), "") == 0 || strcmp(Info_ValueForKey(userinfo, "forcepowers"), "") == 0 || 
		strcmp(Info_ValueForKey(userinfo, "color1"), "") == 0))
	{ // client doesn't have JKA specific info -> fake !
		G_LogPrintf("Q3Fill attack by %s has been banned.\n", gaPortlessIP);
		trap_SendConsoleCommand(EXEC_NOW, va("addip %s\n", gaPortlessIP));
		return "We have detected a Q3Fill attack and your IP has been automatically banned.";
	}
...
//	areabits = client->areabits;
	...
	strcpy(ent->client->sess.myip, gaPortlessIP);
...
//		client->areabits = G_Alloc( (trap_AAS_PointReachabilityAreaIndex( NULL ) + 7) / 8 );
	...
	if (!isBot){
	trap_SendServerCommand( ent-g_entities, va("print \"\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n^1Welcome to JK3 Arena! ^2Type in /help if you need help.\n\"" ) );
	//Arena: A lot of ppl may get annoyed because it may call this more than once on connection enter...
	//loginplz (ent);
	}
...
void ClientBegin( int clientNum, qboolean allowTeamReset ) {
	...
	ent->client->pers.cant_register = qfalse;
	ent->client->pers.can_kill = qfalse;
...
void ClientDisconnect( int clientNum ) {
	...
	if ( (g_gametype.integer == GT_DUEL )
		...
		if ( level.sortedClients[1] == clientNum ) {
			...
			level.clients[ level.sortedClients[1] ].sess.todays_duel_wins++;
	...
	//Update the mysql database before you go.
	...
	ent->client->sess.loggedin = qfalse; //We have to log this clientID out so that when a new person joins, they can login as their own.
	ent->r.contents = 0;
