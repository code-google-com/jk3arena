...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while ( client->timeResidual >= 1000 ) 
	{
		...
		//Arena: Things we need to do every 1 second.
		if (client->csTimeLeft >= 1){
			client->csTimeLeft++;
		}
		...
		if (strcmp(ent->client->pers.experience_gain, "^4Flag Captured!\n^3+10") == 0 ||
			...
			if (client->csTimeLeft == 3){
				client->csTimeLeft = 0;
			}
			if (client->csTimeLeft == 2){
				trap_SendServerCommand( ent-g_entities, va("cp \"%s\n\"", ent->client->pers.experience_gain ) );
			}
		} else {
			if (client->csTimeLeft == 5){
				client->csTimeLeft = 0;
			}
			if (client->csTimeLeft == 4){
				trap_SendServerCommand( ent-g_entities, va("cp \"%s\n\"", ent->client->pers.experience_gain ) );
			}
		}
		}
...
//		G_Printf("serverTime >>>>>\n" );
	...
	if ( client->noclip ) {
		...
		{
			if (!strcmp (client->sess.rank, "Youngling") && client->sess.todays_experience == 500) {
				client->csTimeLeft = 1;
				strcpy(client->pers.experience_gain, va("^5You ranked up to\n^2Padawan^5!\n"));
				strcpy(ent->client->sess.rank, va("Padawan"));
			} else if (!strcmp (client->sess.rank, "Padawan") && client->sess.todays_experience == 1000) {
				client->csTimeLeft = 1;
				strcpy(client->pers.experience_gain, va("^5You ranked up to\n^4Knight^5!\n"));
				strcpy(ent->client->sess.rank, va("Knight"));
			} else if (!strcmp (client->sess.rank, "Knight") && client->sess.todays_experience == 2000) {
				client->csTimeLeft = 1;
				strcpy(client->pers.experience_gain, va("^5You ranked up to\n^3Council Member^5!\n"));
				strcpy(ent->client->sess.rank, va("Council Member"));
			} else if (!strcmp (client->sess.rank, "Knight") && client->sess.todays_experience == 2000) {
				client->csTimeLeft = 1;
				strcpy(client->pers.experience_gain, va("^5You ranked up to\n^3Council Member^5!\n"));
				strcpy(ent->client->sess.rank, va("Council Member"));
			} else if (!strcmp (client->sess.rank, "Council Member") && client->sess.todays_experience == 4000) {
				client->csTimeLeft = 1;
				strcpy(client->pers.experience_gain, va("^5You ranked up to\n^7High Master^5!\n"));
				strcpy(ent->client->sess.rank, va("High Master"));
			} else if (!strcmp (client->sess.rank, "High Master") && client->sess.todays_experience == 6000) {
				client->csTimeLeft = 1;
				strcpy(client->pers.experience_gain, va("^5You ranked up to\n^7Supreme Master^5!\n"));
				strcpy(ent->client->sess.rank, va("Supreme Master"));
			} else if (!strcmp (client->sess.rank, "Supreme Master") && client->sess.todays_experience == 8000) {
				client->csTimeLeft = 1;
				strcpy(client->pers.experience_gain, va("^5You ranked up to\n^3*^1Grand Master^5!^3*\n"));
				strcpy(ent->client->sess.rank, va("Grand Master"));
			}
			if (client->jetPackOn)
			{
...
#define	MIN_NPC_SPEED	16.0f
	...
	if (ent->client->ps.duelInProgress)
	{
		...
		else if (duelAgainst->health < 1 || duelAgainst->client->ps.stats[STAT_HEALTH] < 1)
		{
			...
			ent->client->sess.todays_duel_wins++;

			duelAgainst->client->sess.todays_duel_losses++;
