...
void siegeTriggerUse(gentity_t *ent, gentity_t *other, gentity_t *activator)
{
	...
	if (activator->client->sess.loggedin == qtrue){
	activator->client->csTimeLeft = 1;
	strcpy(activator->client->pers.experience_gain, va("^1Objective Completed!\n^3+30"));
	activator->client->sess.todays_objectives += 1;
	activator->client->sess.todays_experience += 30;
	}
