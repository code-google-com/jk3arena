...
int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	if ( ent->flags & FL_DROPPED_ITEM ) {
		...
		other->client->csTimeLeft = 1;
		strcpy(other->client->pers.experience_gain, va("^2Flag Recovered!\n^3+10"));
		other->client->sess.todays_returns += 1;
		other->client->sess.todays_experience += 10;
		}
...
//	Team_ForceGesture(other->client->sess.sessionTeam);
	...
	if (other->client->sess.loggedin == qtrue){
	other->client->csTimeLeft = 1;
	other->client->sess.todays_captures += 1;
	strcpy(other->client->pers.experience_gain, va("^4Flag Captured!\n^3+10"));
	other->client->sess.todays_experience += 10;
	}
	...
	for (i = 0; i < g_maxclients.integer; i++) {
		...
		if (player->client->sess.sessionTeam !=
			...
			if (player->client->pers.teamState.lastreturnedflag + 
				...
				if (player->client->sess.loggedin == qtrue){
				player->client->csTimeLeft = 1;
				strcpy(player->client->pers.experience_gain, va("^5Flag Assist!\n^3+10"));
				player->client->sess.todays_experience += 10;
				}
