...
void G_AddPowerDuelScore(int team, int score)
{
	...
	while (i < MAX_CLIENTS)
	{
		...
		{ //found a living client on the specified team
			...
			check->client->sess.todays_duel_wins += score;
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	if (meansOfDeath != MOD_SUICIDE && meansOfDeath != MOD_SPECTATE){
		self->client->sess.todays_deaths++;
		if (g_gametype.integer == GT_DUEL || g_gametype.integer == GT_POWERDUEL){
				self->client->sess.todays_duel_losses++;
		}
	}
	if (attacker && attacker->inuse && attacker->client){
			attacker->client->sess.todays_kills++;
			attacker->client->csTimeLeft = 1;
			if (g_gametype.integer == GT_DUEL || g_gametype.integer == GT_POWERDUEL){
				attacker->client->sess.todays_duel_wins++;
			}
		if (attacker->client->sess.loggedin == qtrue){
		if (g_gametype.integer == GT_FFA){
			strcpy(attacker->client->pers.experience_gain, va("^3+10"));
			attacker->client->sess.todays_experience += 10;
		} else {
			strcpy(attacker->client->pers.experience_gain, va("^3+5"));
			attacker->client->sess.todays_experience += 5;
		}
		}
	}
