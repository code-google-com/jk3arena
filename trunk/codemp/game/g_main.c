...
// DUEL_HEALTH
	...
	//Arena: My cvars
	{ &am_email, "am_email", "", 0, 0, qfalse },
	{ &rconSE_password, "rconSE_password", "", 0, 0, qfalse },
	{ &am_sqlpass, "am_sqlpass", "", 0, 0, qfalse },
};
...
void G_RegisterCvars( void ) {
	...
	//arena: cant support these gametypes.
	else if (g_gametype.integer == GT_DUEL)
	{
		G_Printf( "This gametype is not supported on the jk3arena mod.\n" );
		trap_Cvar_Set( "g_gametype", "0" );
	}
...
void AdjustTournamentScores( void ) {
		...
		level.clients[level.sortedClients[1]].pers.connected == CON_CONNECTED)
	{
		...
		if (clFailure != 2)
		{
			...
			level.clients[ clientNum ].sess.todays_duel_losses++;
			level.clients[ clientNum ].sess.todays_deaths++;
			ClientUserinfoChanged( clientNum );
		}
		else
		{
...
void CheckExitRules( void ) { //Arena: This is where the ending conditions are made.
 	int			i;
	gclient_t	*cl;
	char *sKillLimit;
	qboolean printLimit = qtrue;
	char	s[MAX_STRING_CHARS];
	char stuff[32][MAX_STRING_CHARS];
	char *time;
	...
	if (g_gametype.integer != GT_SIEGE)
	{
		if ( g_timelimit.integer && !level.warmupTime ) {
			if ( level.time - level.startTime >= g_timelimit.integer*60000 ) {
				if ((g_gametype.integer != GT_DUEL) && (g_gametype.integer != GT_POWERDUEL)){
					trap_Cvar_VariableStringBuffer( "sv_hostname", s, sizeof(s) );
					for(i = 0; i < level.maxclients; i++) {
						if(g_entities[i].client->pers.connected == CON_CONNECTED) 
						{
							Com_sprintf(stuff[i], sizeof(stuff[i]), "Name:%s Kills:%i Deaths:%i Team:%s Duel Wins:%i Duel Losses:%i Flags Captured:%i Flags Assists:%i Flags Returns:%i Siege Objectives Complete:%i<br>", g_entities[i].client->pers.netname, g_entities[i].client->sess.todays_kills, g_entities[i].client->ps.persistant[PERS_KILLED], TeamName(g_entities[i].client->sess.sessionTeam), g_entities[i].client->sess.todays_duel_wins, g_entities[i].client->sess.todays_duel_losses, g_entities[i].client->sess.todays_captures, g_entities[i].client->sess.todays_returns, g_entities[i].client->sess.todays_objectives);
						}
					}
					//Arena: post the game
				}
