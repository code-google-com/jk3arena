...
void Cmd_Score_f( gentity_t *ent ) {
	...
	if (ent->client->sess.loggedin == qtrue){
		trap_SendServerCommand( ent-g_entities, va("cp \"^7[ %s ^7]\n^2RANK: %s\n^0----------------------\n^7Deaths: %i\nKills: %i\nFlag Captures: %i\nFlag Returns: %i\nDuel Wins: %i\nDuel Losses: %i\nSiege Objectives: %i\n^0----------------------\n^3EXPERIENCE: %i\n\"",
			...
			ent->client->sess.todays_returns, ent->client->sess.todays_duel_wins, ent->client->sess.todays_duel_losses, ent->client->sess.todays_objectives, ent->client->sess.todays_experience ) );
	}
}
...
==================
Arena: MakeMD5
Created 6/3/2010 by NeWaGe
Takes a word, and makes it MD5 encrypted.
==================
*/
char *MakeMD5 ( char *word ){
	unsigned char digest[16] = {""};
	unsigned int i;
	static char total[MAX_STRING_CHARS];
	char frac1[4];char frac2[4];char frac3[4];char frac4[4]; // These fractions come in pairs, I want 4 just incase.
	char frac5[4];char frac6[4];char frac7[4];char frac8[4];
	char frac9[4];char frac10[4];char frac11[4];char frac12[4];
	char frac13[4];char frac14[4];char frac15[4];char frac16[4];
	MD5_CTX context;

	MD5Init(&context);
	MD5Update(&context, word, strlen(word));
	MD5Final(digest, &context);

	//MD5 displays in 16 parts, each group holds a set of information
	for (i = 0; i < 16; i++) {
		Com_sprintf(frac1, sizeof(frac1), "%02x", digest[0]);
		Com_sprintf(frac2, sizeof(frac2), "%02x", digest[1]);
		Com_sprintf(frac3, sizeof(frac3), "%02x", digest[2]);
		Com_sprintf(frac4, sizeof(frac4), "%02x", digest[3]);
		Com_sprintf(frac5, sizeof(frac5), "%02x", digest[4]);
		Com_sprintf(frac6, sizeof(frac6), "%02x", digest[5]);
		Com_sprintf(frac7, sizeof(frac7), "%02x", digest[6]);
		Com_sprintf(frac8, sizeof(frac8), "%02x", digest[7]);
		Com_sprintf(frac9, sizeof(frac9), "%02x", digest[8]);
		Com_sprintf(frac10, sizeof(frac10), "%02x", digest[9]);
		Com_sprintf(frac11, sizeof(frac11), "%02x", digest[10]);
		Com_sprintf(frac12, sizeof(frac12), "%02x", digest[11]);
		Com_sprintf(frac13, sizeof(frac13), "%02x", digest[12]);
		Com_sprintf(frac14, sizeof(frac14), "%02x", digest[13]);
		Com_sprintf(frac15, sizeof(frac15), "%02x", digest[14]);
		Com_sprintf(frac16, sizeof(frac16), "%02x", digest[15]);
	}
	//We must save and put all the fractions together
	Com_sprintf(total, sizeof(total), "%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s", frac1, frac2, frac3, frac4, frac5, frac6
		, frac7, frac8, frac9, frac10, frac11, frac12, frac13, frac14, frac15, frac16);
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	//No cursing either, these servers are for everyone.
	if ( strstr(text, "nigger")){
			trap_SendServerCommand( ent-g_entities, va("print \"Nigger is a badword.\n\"" ) );
			return;
	} 
	if ( strstr(text, "bitch")){
			trap_SendServerCommand( ent-g_entities, va("print \"Bitch is a badword.\n\"" ) );
			return;
	} 
	if ( strstr(text, "asshole")){
			trap_SendServerCommand( ent-g_entities, va("print \"Asshole is a badword.\n\"" ) );
			return;
	} 
	if ( strstr(text, "vagina")){
			trap_SendServerCommand( ent-g_entities, va("print \"Vagina is a badword.\n\"" ) );
			return;
	} 
	if ( strstr(text, "pussy")){
			trap_SendServerCommand( ent-g_entities, va("print \"Pussy is a badword.\n\"" ) );
			return;
	} 
	if ( strstr(text, "penis")){
			trap_SendServerCommand( ent-g_entities, va("print \"Penis is a badword.\n\"" ) );
			return;
	} 
	if ( strstr(text, "fuck")){
			trap_SendServerCommand( ent-g_entities, va("print \"Fuck is a badword.\n\"" ) );
			return;
	}
...
void ClientCommand( int clientNum ) {
	...
	else if (Q_stricmp (cmd, "register") == 0)
	{
		...
		if ( trap_Argc() != 4 ) 
        { 
			trap_SendServerCommand( clientNum, va("print \"Usage: /register <username> <password> <email>\n\"" ) );
			return;
        }

		if (ent->client->sess.loggedin == qtrue) {
			trap_SendServerCommand( clientNum, va("print \"You are currently logged in.\n\"" ) );
			return;
		}

		if (strcmp(ent->client->sess.myip, "") == 0){
			trap_SendServerCommand( clientNum, va("print \"You do not have an IP address. Please reconnect to the server. This is not a bug.\n\"" ) );
			return;
		}
		...
		trap_SendServerCommand( ent-g_entities, va("print \"%s\n\"", Curl_Address( va("http://clanmod.org/jk3arena.com/server/database.php?pass=%s&register_info&username=%s&password=%s&email=%s&myip=%s", am_sqlpass.string, username, password, email, ent->client->sess.myip )) ) );
	...
	else if (Q_stricmp (cmd, "logout") == 0)
	{
		if (ent->client->sess.loggedin == qfalse) {
			trap_SendServerCommand( clientNum, va("print \"You are not logged in.\n\"" ) );
			return;
		}
		...
		strcpy(ent->client->sess.username, va(" "));
		strcpy(ent->client->sess.password, va(" "));
		ent->client->sess.todays_deaths = 0;
		ent->client->sess.todays_kills = 0;
		ent->client->sess.todays_experience = 0;
		ent->client->sess.todays_captures = 0;
		ent->client->sess.todays_returns = 0;
		ent->client->sess.todays_duel_losses = 0;
		ent->client->sess.todays_duel_wins = 0;
		ent->client->sess.todays_objectives = 0;
	...
	{
		if (ent->client->sess.loggedin == qtrue){
			trap_SendServerCommand( ent-g_entities, va("print \"^5Account: %s\n^2RANK: %s\n^0----------------------\n^7Deaths: %i\nKills: %i\nFlag Captures: %i\nFlag Returns: %i\nDuel Wins: %i\nDuel Losses: %i\nSiege Objectives: %i\n^0----------------------\n^3EXPERIENCE: %i\n^4Your IP address: %s\n\"",
				ent->client->sess.username, ent->client->sess.rank, ent->client->sess.todays_deaths, ent->client->sess.todays_kills, ent->client->sess.todays_captures,
	...
	/*else if (Q_stricmp (cmd, "resetaccount") == 0)
	{
		...
		char   reset[MAX_STRING_CHARS];
		if (ent->client->sess.loggedin == qfalse){
			trap_SendServerCommand( ent-g_entities, va("print \"You are not logged in.\n\"" ) );
			return;
		}
		if (ent->client->updateTimeLeft != 0){
			trap_SendServerCommand( ent-g_entities, va("print \"Please wait 30 seconds to reset again. Time Left: %i\n\"", ent->client->updateTimeLeft ) );
			return;
		}
		if (trap_Argc() != 2) 
        { 
			trap_SendServerCommand( clientNum, va("print \"Usage: /resetaccount <yes>\n^1WARNING: This will reset all your information except username, password, email, and posts.\n\"" ) );
			return;
        }
		trap_Argv( 1, reset, sizeof( reset ) ); // yes|no
		if ( Q_stricmp( "yes", reset ) == 0 )
	{
